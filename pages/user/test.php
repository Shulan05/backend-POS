...
$manage_user = getUserByID($_GET['id']); // get user by id,object
...
if (isset($_POST['user_label']) && isset($_POST['username']) && isset($_POST['passwd'])) {
    ...
    if (!empty($username) && usernameExists($username)) {
        $username_err = "Username is already exists!";
    }
...
<form action="./?page=user/update&id=<?php echo $manage_user->id_user ?>" method="post" class="w-50 mx-auto">
    ...
    <div class="mb-3">
        ...
        <input type="password" name="passwd" placeholder="(optional) input new password to update" class="form-control"
